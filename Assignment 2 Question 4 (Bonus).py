...
while (flag==False and tries<6):
    ...
    word_duplicate = list(word)
    url = "https://api.dictionaryapi.dev/api/v2/entries/en/"
    validity = requests.get("https://api.dictionaryapi.dev/api/v2/entries/en/"+s)

    if len(s)!=5:
        print("I TOLD YOU TO ENTER A 5 DIGIT NUMBER! TRY AGAIN!")
        continue

    elif len(validity.json())==3:
        print("YOU NEED TO TYPE A VALID WORD!!")
        continue
        ...
        for i in range(len(s)):
            if s[i]==word[i]:
                output[i] = s[i]
                if s[i] in word_duplicate:
                    word_duplicate.remove(s[i])
